...
export default {
    ...
    async _render() {
        ...
        await Promise.all(_.map(this.matches, (match) => match.render(matchContainer.group())))
        ...
        paths = _.map(this.matches, (match_1) => this.makeCurve(containerBox, match_1))
        ...
        this.container.prepend(this.container.path(_(paths).flatten().compact().values().join('')))
        containerBox = matchContainer.getBBox()
        // Create connections from side lines to each match and render into
        // the match container.
        paths = _.map(this.matches, (match_2) => this.makeConnector(containerBox, match_2))
    ...
    setup() {
        if (this.properties.alternates.elements.length === 0) {
            ...
            this.matches = [this.properties.match].concat(
                _.map(this.properties.alternates.elements, (element) => element.properties.match),
            )
        }
    },
}
