// =============================================================================
// 使用 canopy@0.2.0 的命令行工具编译 ./grammar.peg 文件，
// 并需要将编译后的 JS 文件手动修改为 es module 格式。
// =============================================================================
...
var find = function (root, objectName) {
    ...
    while ((part = parts.shift())) {
        root = root[part]
        if (root === undefined) throw new Error('Cannot find object named ' + objectName)
    }
    return root
}
    ...
    __consume__root: function (input) {
        ...
        if (slice0 === '/') {
            ...
            if (!this.error || this.error.offset <= this._offset) {
                this.error = this.constructor.lastError = {
                    input: this._input,
                    offset: this._offset,
                    expected: '"/"',
                }
...
export default { Grammar, Parser, parse: Parser.parse }
